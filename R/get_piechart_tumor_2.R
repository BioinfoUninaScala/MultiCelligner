#' 
#' Create a pie chart that illustrates the percentage of the lineages belong to tumor k nearest neighbors samples for the metasample
#'
#' @import dplyr
...
#' @return a pie chart illustrating the distribution of the TCGA k nearest neighbors lineages to the metasample 
#' @export

get_piechart_tumor_2 <- function(combined_mat, selected_samples, n, ann) {
  ...
  dist_top_n_1 <- dist_metasmaple_1 %>% filter(grepl('TCGA', sampleID))

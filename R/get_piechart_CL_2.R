#' 
#' Create a pie chart that illustrates the percentage of the lineages belong to CL k nearest neighbors samples for the metasample
#'
#' @import dplyr
...
#' @return a pie chart illustrating the distribution of the CL k nearest neighbors lineages to the metasample 
#' @export

get_piechart_CL_2 <- function(combined_mat, selected_samples, n, ann) {
  ...
  y <-  ggplot(dist_top25_4, aes(x = "", y = Freq, fill = lineage_ccle)) +
    ...
    geom_text(aes(label = label), 
              ...
              size = 5) +
    labs(title = paste("Distribution of Lineage CCLE for metasample"))

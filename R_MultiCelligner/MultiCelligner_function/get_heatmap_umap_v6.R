...
get_heatmap_umap_v6 <- function(mnn_param, CCLE_cor, TCGA_cor, TCGA_ann, CCLE_ann, comb_ann, subset_genes) {
  ...
  dist_top25_lin <- dplyr::left_join(dist_df_top25, TCGA_ann[, c(1,3)], by = "sampleID") #ann_multiomics
  colnames(dist_top25_lin)[4] <- "lineage_tcga"
  
  dist_top25_lin <- dplyr::left_join(dist_top25_lin, CCLE_ann[, c(1,3)], by = "ref_ID", ) #ann_multiomics_1
